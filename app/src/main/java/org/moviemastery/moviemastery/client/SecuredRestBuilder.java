...
public class SecuredRestBuilder extends RestAdapter.Builder {
 	...
 	private class OAuthHandler implements RequestInterceptor {
		...
		public void intercept(RequestFacade request) {
            ...
            Log.w("TAG", "Are we loggged in? " + loggedIn);
            System.out.println("Are we loggged in? " + loggedIn);
            if (!loggedIn) {
                try {
                    ...
                    if (resp.getStatus() < 200 || resp.getStatus() > 299) {
                        ...
                        // correct or not..???
//                        Log.w("TAG", resp.getBody().toString());
...
//                        request.addHeader("Authorization", "Bearer " + accessToken);
            ...
            Log.w("TAG", "Are we loggged in after? " + loggedIn);
        }
